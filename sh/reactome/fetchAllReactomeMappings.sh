...
cat `dirname $0`/UniProt2PlantReactome_All_Levels.txt | awk -F"\t" '{print $1"\t"$6"\t"$2"\t"$4}' | sort -k 1,1 > aux.UniProt2PlantReactome
cat `dirname $0`/Ensembl2PlantReactome_All_Levels.txt | awk -F"\t" '{print $1"\t"$6"\t"$2"\t"$4}' | sort -k 1,1 > aux.Ensembl2PlantReactome
#Download the other files
curl -s -X GET "http://www.reactome.org/download/current/UniProt2Reactome_All_Levels.txt" | awk -F"\t" '{print $1"\t"$6"\t"$2"\t"$4}' | sort -k 1,1 > aux.UniProt2Reactome
...
curl -s -X GET "http://www.reactome.org/download/current/Ensembl2Reactome_All_Levels.txt" | awk -F"\t" '{print $1"\t"$6"\t"$2"\t"$4}' | grep -v '^ENST' | sort -k 1,1 > aux.Ensembl2Reactome
...
for file in UniProt2Reactome UniProt2PlantReactome; do
  ...
  for organism in $(cat aux.$file.organisms); do
    ...
    if [ -s $uniprotMappingFile ]; then
      ...
      grep -v '\t$' $uniprotMappingFile | awk -F"\t" '{print $2"\t"$1}' | sort -k1,1 | uniq > aux.${lcOrganism}.ensembl.tsv.tmp
