...
for l in $(grep -P '^ID|^AC|\/accession="|\/product="' ${outputDir}/miRNA.dat); do
    if [[ $l == ID* ]]; then
	hairpinIdentifier=`echo $l | awk '{print $2}'`
    elif [[ $l == AC* ]]; then	
	hairpinAccession=`echo $l | awk '{print $2}' | sed '$s/;$//'`
    elif [[ $l == FT*/accession=* ]]; then	
	matureAccession=`echo $l | awk -F"=" '{print $2}' | sed '$s/"$//' |  sed '$s/^"//'`
...
for f in mature hairpin; do
   ...
   for l in $(cat ${outputDir}/${f}.fa); do
       mirbase_id=`echo $l | awk '{print $1}' | awk -F">" '{print $NF}'`
       mirbase_accession=`echo $l | awk '{print $2}'`
       if [ $f == "mature" ]; then
	   ...
	   # via the hairpin id corresponding to $mirbase_id in ${outputDir}/hairpin_to_mature.tsv
       ...
       if [ $f == "mature" ]; then
	      echo -e "$mirbase_id\t$mirbase_accession\t$mirbase_accession\t$mirbase_name\t$mirbase_sequence\t$hairpinIdentifier\tmiRNA" >> ${outputDir}/${organism}.${f}.tsv
       else 
	      echo -e "$mirbase_id\t$mirbase_accession\t$mirbase_accession\t$mirbase_name\t$mirbase_sequence\tmiRNA" >> ${outputDir}/${organism}.${f}.tsv
       fi
   done
done
